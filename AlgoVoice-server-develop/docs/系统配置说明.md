...
database:
  type: duckdb                   # 数据库类型
  path: "data/AlgoVoice.db"        # 数据库文件路径
...
**重要说明**:
- 系统采用**主备容错机制**
- **主服务**: AI Model 本地模型（优先使用）
- **备用服务**: 阿里云API（AI Model失败时自动切换）
...
参见: `module_10_ai_interaction/config/AI_Model_config.yaml`

```yaml
model:
  model_path: ".AI Model"          # 模型文件路径
...
修改 `module_10_ai_interaction/config/AI_Model_config.yaml`:
...
# 数据库配置
DATABASE_PATH=data/AlgoVoice.db
...
### Q2: 如何禁用AI Model只使用阿里云？

A: 两种方法：
1. 删除或重命名 `.AI Model` 目录
2. 系统会自动检测并切换到阿里云
