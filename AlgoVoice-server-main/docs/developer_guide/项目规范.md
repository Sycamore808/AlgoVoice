...
`AlgoVoice` 是一个AI Model 赋能的自适应量化投资引擎，金织AlgoVoice “金融（Fin）+ 编织（Loom）”，寓意系统像织布机一样，将数据、因子、模型、用户需求等元素编织成个性化的投资组合，科技感与艺术感兼具。

系统以 **AI Model 模型** 作为自然语言理解入口，支持用户通过文本输入表达投资需求（如“找高成长性中小盘股”），系统自动解析并生成对应的量化策略。
...
|------|------|------|
| Python | 3.9+ | 主语言 |
| PyTorch | 2.0+ | 仅用于 AI Model 模型推理（非训练） |
| SQLite  |      | 数据库                           |
|         |      |                                  |
|         |      |                                  |
